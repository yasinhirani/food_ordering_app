...
const Cart = () => {
  ...
  return (
    <div className="flex-grow h-full w-full max-w-baseWidth mx-auto px-6 md:px-12 py-6 mt-20 overflow-x-hidden">
      <h4 className="text-2xl font-bold">
        Your Shopping Cart {cartItems.length === 0 ? "is empty." : ""}
      </h4>
      <div className="overflow-x-auto">
        {cartItems.length > 0 && (
          <table className="w-full mt-10 cart-table">
            <thead>
              <tr>
            ...
            <tbody>
              {cartItems.map((item) => (
                <tr key={Math.random()}>
                  <td className="tbody">
                    <figure>
                      <img src={item.itemImage} alt="" className="w-36 h-24 min-w-[9rem]" />
                    </figure>
                  </td>
                  <td className="tbody">{item.itemName}</td>
                  <td className="tbody">RS.{item.price}</td>
                  <td className="tbody">
